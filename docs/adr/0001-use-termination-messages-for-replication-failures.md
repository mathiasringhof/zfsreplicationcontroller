...
- When multiple Pods exist, the controller considers only Pods owned by the exact Job UID and deterministically chooses the newest terminated `sender` container by finish time, creation time, then name.
